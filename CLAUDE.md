...
ENVIRONMENT=development uv run uvicorn tjpw_schedule_watcher.api:app --reload --port 8000
...
# 全インターフェースでリッスン
uv run uvicorn tjpw_schedule_watcher.api:app --host 0.0.0.0
...
```
src/tjpw_schedule_watcher/         # メインアプリケーションコード
...
2. 実装: `src/tjpw_schedule_watcher/`
...
1. 標準ライブラリ
2. サードパーティライブラリ
3. ローカルモジュール（`from . import`、`from tjpw_schedule_watcher import`）
